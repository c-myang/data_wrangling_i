...
`==` is the test for equality, NOT a single equal sign, which is used to
assign variables!
...
Putting `!` outside logical operation gives you the inverse.
...
Putting `!` outside logical operation is better for more complicated
logical statements.
...
Real challenge in using filter is coming up with a way to specify the
observations you want to keep!
